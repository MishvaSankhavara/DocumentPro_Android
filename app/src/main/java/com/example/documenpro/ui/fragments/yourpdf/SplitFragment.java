...
public class SplitFragment extends Fragment implements OnPdfClickListener {
    ...
    public SplitFragment(ResultViewerActivity mActivity) {
    ...
    public static class LoadFile extends AsyncTask<Void, Void, Void> {
        WeakReference<SplitFragment> weakReference;

        public LoadFile(SplitFragment compressFragment) {
            this.weakReference = new WeakReference<>(compressFragment);
        }

        @Override
        protected Void doInBackground(Void... voids) {
            this.weakReference.get().arrayList = Utils.getCreatedPdf(GlobalConstant.RootDirectorySplitSaved);
            return null;
        }
